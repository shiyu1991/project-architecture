...
**调研频率：**
- 每个选型层级在展示候选前都必须执行一次 `web_search` 调研
- 同一会话内同一层级不重复调研（结果缓存）
- 调研结果不写入 skill 文档，仅用于当前选型会话
...
- 每层给出 **Top 3 常用候选**（文档中的表格为编写时参考基线，AI Agent 必须按 0.4 协议动态调研后合并展示最新候选）
- 用户可**跳过推荐**，自主输入任意方案
...
**规则：** 从 Layer 1 开始逐层选择，前序选择影响后序推荐。每层默认单选，AI Agent 按「0.5 多选兼容评估协议」评估后可开放多选。
...
**多选评估：** ❌ 互斥 — Lint/Format 工具规则会冲突，仅可单选（ESLint + Prettier 作为一组方案）。
...
> **强制规则：** 后端选型在阶段三进行，必须在前端选型全部完成后才开始。禁止与前端层级混批。每层默认单选，AI Agent 必须按「0.5 多选兼容评估协议」评估该层是否适合多选。
...
> **新兴运行时（Node.js 生态）：** Bun（极速启动、内置打包/测试/包管理）和 Deno（安全沙箱、原生 TypeScript、Web 标准 API）正在快速发展。选择 Node.js 时，AI Agent 应在选型结果中提示用户可考虑 Bun/Deno 作为运行时替代，但需标注"新兴，生产环境建议观察"。

**选择信号：** 大型企业系统 / 国内传统企业 → Java；前后端同构 / 快速迭代 → Node.js；高并发 / 云原生 / 基础设施工具 → Go。

**多选评估：** ❌ 互斥 — 一个后端服务只能用一种主语言，仅可单选。版本一律选择当前 LTS（按 0.1 协议实时确认）。
