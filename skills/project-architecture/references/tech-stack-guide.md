...
- 选型分四阶段导航：阶段一核心跨端项，阶段二前端层级，阶段三后端层级，阶段四跨端关注点；插件和扩展推荐不固定在最后，而是在每次选择确认后动态插入下一轮
- **前端和后端选型必须分离**：先完整选完前端所有层级，再选后端所有层级，禁止同一轮问题混合前后端层级，且每轮只能包含一个问题
...
用户完成各技术层级选择后，AI Agent 根据已选结果触发配套插件推荐。每个插件必须作为独立选项展示；表格中的多个已选技术仅表示触发条件，不是一个可选组合，也不得自动勾选任何插件。
...
| 已选技术条件 | 独立插件选项 | 用途 | 推荐级别 |
|--------------|--------------|------|----------|
| Vue + Pinia | `pinia-plugin-persistedstate` | 状态持久化 | 推荐 |
| Vue + Vite | `unplugin-auto-import` | API 自动导入 | 推荐 |
| Vue + Vite | `unplugin-vue-components` | 组件自动导入 | 推荐 |
...
| 已选技术条件 | 独立插件选项 | 用途 | 推荐级别 |
|--------------|--------------|------|----------|
| NestJS | `@nestjs/swagger` | Swagger 文档 | 推荐 |
| NestJS | `@nestjs/throttler` | 限流保护 | 推荐 |
| NestJS | `@nestjs/passport` | 认证策略 | 启用认证时强烈推荐，仍由用户确认 |
...
用户完成后端各层级选择后，AI Agent 根据已选结果推荐配套插件。每个插件必须作为独立选项展示；表格中的多个技术仅表示触发条件，不是固定组合选项，也不得自动勾选。
